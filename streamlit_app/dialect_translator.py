...
def load_classification_model():
    with st.spinner("Loading model for classification..."):
        ...
        model_url = "https://github.com/GirinMan/HAI-DialectTranslator/raw/main/multi_label_classification/train/model_%ED%99%A9%ED%83%9C%EA%B2%BD.pth"
        ...
        model = AutoModelForSequenceClassification.from_pretrained(model_ckpt, num_labels=4).to('cuda')
...
def classification(model, tokenizer, input_txt):
    ...
    result = np.argmax(preds[:,:3], axis=1).item()
...
def translation(model, tokenizer, input_txt):
    ...
    generated_ids = model.generate(input_tensor, num_beams=4,  no_repeat_ngram_size = 2)
...
def main():
    ...
    st.subheader("한국어 방언 번역기")
    st.markdown("""
        #### Description
        - 이 웹 애플리케이션은 PyTorch와 Transformers 라이브러리를 활용하여 표준어와 경상도, 제주도 방언을 자동으로 인식하고, 번역해주는 프로그램 입니다.
        - 모델 서빙 및 현재 보여지는 웹 페이지는 Streamlit을 활용하여 구현되었습니다.
        - 2022-2학기 HAI 1팀(권예준, 김한결, 이성진, 장서연, 주영민, 한상엽, 황인서, 황태경)
        """)

    class_model, class_tokenizer = load_classification_model()
    trans_models = load_translation_models()

    options = ['표준어', '경상도 방언', '제주도 방언']
    ...
    if st.button("번역하기"):
        ...
        elif init == 0 and target == 2:
            selected = trans_models[2]
        elif init == 2 and target == 0:
            selected = trans_models[3]
        elif init == 1 and target == 2:
            message = translation(trans_models[1][0], trans_models[1][1], message)
            selected = trans_models[2]
        elif init == 2 and target == 1:
            message = translation(trans_models[3][0], trans_models[3][1], message)
            selected = trans_models[0]
